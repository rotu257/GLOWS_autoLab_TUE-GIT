...
- --plot flag (plots scaled data after acquisition)

Acquisition flow:
- Replaces :DIGitize with:
    :RUN
    :SINGLE
...
class KeysightInfiniiVision6000X:
    ...
    def wait_for_single_acquisition(self, timeout_sec: float = 300.0) -> None:
        """
        Wait for the :SINGLE acquisition to complete.

        We first try trigger state, and fall back to *OPC?.
        On many InfiniiVision scopes, :TRIGger:STATus? returns STOP when
        the single acquisition has completed.
        """
        start = time.time()
        while time.time() - start < timeout_sec:
            try:
                state = self.query(":TRIGger:STATus?").strip().upper()
                if state == "STOP":
                    logger.info("Single acquisition complete (TRIGger:STATus = STOP)")
                    return
            except Exception:
                pass
            ...
            try:
                if self.query("*OPC?") == "1":
                    logger.info("Single acquisition complete (*OPC? = 1)")
                    return
            except Exception:
                pass
        ...
        logger.warning(f"Single acquisition wait timeout after {timeout_sec:.1f} s — continuing anyway")
    ...
    def acquire_and_save(self) -> None:
        ...
        logger.info("Preparing acquisition: :TRIGger:SWEep AUTO")
        self.write(":TRIGger:SWEep AUTO")

        logger.info("Setting scope to RUN")
        self.write(":RUN")
        time.sleep(0.2)

        logger.info("Arming single acquisition")
        self.write(":SINGLE")
        time.sleep(0.2)

        logger.info("Forcing trigger to complete single acquisition")
        self.write(":TRIGger:FORCe")

        self.wait_for_single_acquisition(timeout_sec=300.0)
...
Notes
-----
- The script uses :RUN -> :SINGLE -> wait for completion, then reads waveform data with
  :WAV:DATA?.
