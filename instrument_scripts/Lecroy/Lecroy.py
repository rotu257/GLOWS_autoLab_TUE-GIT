...
class Lecroy():
    def __init__(self, channel=None, encoding='BYTE', spe_mode=None, filename=None, FACT=8., query=None, command=None, FORCE=False, PRINT=False, chan_spe=None):
        if encoding == 'BYTE':
            dtype = int8
            NUM = 256
            LIM = 217.  # 15% less than the maximal number possible
        elif encoding == 'WORD':
            dtype = int16
            NUM = 65536
            LIM = 55700.  # 15% less than the maximal number possible
                    ...
                    if channel[i] not in chan_spe:
                        ...
                        while k <= spe_mode:
                            ...
                            while self.query('TRMD?') != 'TRMD STOP':
                                time.sleep(0.05)
                                pass
                            print('Optimization loop index:', k, spe_mode)
...
if __name__ == '__main__':
    ...
    parser.add_option("-e", "--encoding", type="string", dest="encoding", default='BYTE',
                      help="For modifying the encoding format of the answer")
    parser.add_option("-m", "--spe_mode", type="string", dest="spe_mode", default=None,
                      help="For allowing auto modification of the vertical gain. List of: spe_mode iteration number, all the channels to apply spe mode on. Note if no channel precized, all the channel are corrected. WARNING: if you want all the channels to correpond to the same trigger event, you need to spe_mode one channel only and to physically plug the cable in the first channel acquired (first in the list 1->4)")
    parser.add_option("-n", "--spe_fact", type="float", dest="spe_fact", default=8.,
                      help="For setting limits of the vertical gain, units are in number of scope division. Do not overpass 9 due to a security in the code!")
    ...
    if (len(args) == 0) and (options.com is None) and (options.que is None):
        ...
        chan = []
        temp_chan = args[0].split(',')  # Is there a comma?
    ...
    if options.spe_mode:
        spe_mode = eval(options.spe_mode.split(',')[0])
        print('spe_mode value:   ', spe_mode)
        temp_chan_spe = options.spe_mode.split(',')[1:]  # Is there a comma?
        chan_spe = []
        for i in range(len(temp_chan_spe)):
            chan_spe.append('C' + temp_chan_spe[i])
    else:
        spe_mode = None
        chan_spe = None
    
    try:
        a = Lecroy(channel=chan, encoding=options.encoding, spe_mode=spe_mode, filename=options.filename, FACT=options.spe_fact, query=options.que, command=options.com, FORCE=options.force, chan_spe=chan_spe)
    except Exception as e:
        print(e)
        print('\n\n   You should be more carefull with your inputs    \n\n')
        sys.exit()
